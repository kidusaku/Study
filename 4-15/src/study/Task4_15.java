 ...
 * 次のプログラムは、文字列を数値に変換して表示する処理です。 ただし、文字列が数値でない場合にエラーが発生してしまいます。 このプログラムに
 * try-catch-finally を追加して、 エラーが発生してもアプリが止まらずにメッセージを出力するように修正してください。
 *
 * 課題要件 Integer.parseInt(str) の行で例外が発生します。 その処理を try-catch
 * で囲み、NumberFormatException をキャッチしてください。 例外が発生したときは「数値に変換できませんでした。」と表示してください。
